...
import { Home, Clock, Receipt, NotebookPen, User } from "lucide-react";
...
const MobileNavigation = () => {
  ...
  const navItems = [
    { to: "/mobile/dashboard", icon: Home, label: "Start" },
      ...
      <div className="flex justify-around items-center h-24 px-1 pb-safe">
        {navItems.map((item) => (
          <NavLink
            ...
            to={item.to}
            className={({ isActive }) =>
              `flex flex-col items-center justify-center flex-1 min-h-[68px] transition-all duration-200 rounded-2xl px-1 py-3 ${
                isActive
          ...
          >
            {({ isActive }) => (
              <>
                <item.icon className={`h-7 w-7 mb-1 transition-all ${isActive ? "scale-110" : ""}`} />
