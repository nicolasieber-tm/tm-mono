...
- [x] `packages/testimonials` (geteilte Daten, `@tm/testimonials`) angelegt → `Testimonials.tsx`
      (Komponente in `apps/tm/src/components/sections/`) + CSS in `apple-home.css` →
      **in TM-Hauptseite eingebaut** (Section `#stimmen` nach Produkte, vor Team; 4 echte Stimmen
      Mehmet/Sandro/Beat/Luca, André ausgeblendet bis O-Ton, Nav-Link „Stimmen").
      **Filter-Pills bewusst NICHT aktiv** (bei nur 1–2 Stimmen/Kategorie kein Mehrwert; TM zeigt alle,
      Unterseiten via `product`-Prop). Zurückbauen, sobald ~4–5 Stimmen/Produkt — Vorlage `t-filter`
      in `testimonials.html` + auskommentierte Logik-Historie in der Komponente.
      Verifiziert (Puppeteer: 4 Karten, keine Console-/Bild-Fehler).
      ⏳ Noch offen: sichtbarkeit (`product="web"`) / auron / oco einsetzen (Daten liegen im Package bereit).
